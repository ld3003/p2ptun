...
int p2ptun_input_p2pdata_kcp(struct P2PTUN_CONN_SESSION *session, unsigned char *data, int length)
{
    ...
    if (session->kcp)
    {
        ...
        {
            ret = ikcp_send(session->kcp, (const char*)data, length);
...
int p2ptun_input_data(struct P2PTUN_CONN_SESSION *session, unsigned char *data, int length)
{
    ...
    if (length > 1)
    {
        ...
        {
        case '{':
            p2ptun_input_msg(session, (char*)data);
            break;
        case 'D':
        {
        ...
        case 'K':
        {
            ...
            */
            if (session->kcp)
            {
                ikcp_input(session->kcp, (const char*)(data + 1), length - 1);
                ...
                if (kcpdata > 0)
                {
                    ...
                    if (kcpdata_len > 0)
                    {
                        ...
                        session->out_p2pdat_kcp((unsigned char*)kcpdata, kcpdata_len);
                    }
                    free(kcpdata);
                }
                else
